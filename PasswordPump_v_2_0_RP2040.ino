...
//#define SLOW_SPI //orig commented
#define DEBUG_ENABLED             0                                             // <-- set this to 1 if you want debug info written to serial out.
#define TEST_EEPROM               0                                             // <-- set this if you only want to test the eeprom.  Read the code first to understand what this does.

#include <avr/pgmspace.h>
#include <avr/interrupt.h>
#include <Wire.h>
#include "Button2.h"    //#include <Button2.h>                                          // https://github.com/LennartHennigs/Button2 for the button on the rotary encoder 
//#include "Keyboard.h"                                                           // 
#include <Keyboard.h>
...
#define GREEN_PIN                 11
#define RED_PIN                   12//5 //2                                             // Pin locations for the RGB LED, must be PWM capable 
#define BLUE_PIN                  13                                            //   "

#ifdef __RP2040__
  //#define GREEN_PIN               A2                                            //   "
  #define SET_OUT_PIN							A3 /*A4---there is no A4 for rp2040*/
#endif
...
#define SPI_SS_PRIMARY            20 //11                                            // chip select primary (copy source)  (SPI)
#define SPI_SS_SECONDARY          22 //10                                            // chip select secondary (copy target)  (SPI)
...
#define JOYSTICK_SPEED            250                                           // when the joystick is held in one position this is the speed with which the next EVENT_ROTATE event is fired, in milliseconds
...
uint8_t rotaryPin1  = 9;                                                        // also joystick down / up
uint8_t rotaryPin2  = 7;                                                        // also joystick up / down

#if MODEL == MODEL_JOYSTICK
  uint8_t buttonPin = A3; 
#else
  uint8_t buttonPin = 8;   //12 M0
#endif

#if DEFAULT_ORIENT == ORIENT_LEFTY
  uint8_t joystick1 = A1;                                                       // joystick clockwise
  uint8_t joystick2 = 7;                                                        // joystick clockwise
  uint8_t joystick3 = A0;//A5                                                      // joystick counter-clockwise
  uint8_t joystick4 = 9;                                                        // joystick counter-clockwise
#else
  uint8_t joystick1 = A0;//A5                                                       // joystick count-clockwise 
...
unsigned long joyButtonTime1 = 0;
unsigned long joyButtonTime2 = 0;
unsigned long joyButtonTime3 = 0;
unsigned long joyButtonTime4 = 0;
uint8_t joyOldButtonState1 = HIGH;
uint8_t joyOldButtonState2 = HIGH;
uint8_t joyOldButtonState3 = HIGH;
uint8_t joyOldButtonState4 = HIGH;
int joyStickSpeed = JOYSTICK_SPEED;
const unsigned long joyDebounceTime = 100;
...
void setup() {                                                                  // runs first when the device is powered on
  ...
  if(DEBUG_ENABLED) {
    ...
    Serial.print("DEBUG_ENABLED: ");
    while(!Serial);
  }
  
  DebugLN("PasswordPump");

  //`till here it seems to work, screen and keyboard
...
//til here working??
  ...
  switch (orientation) {
    case ORIENT_LEFTY:
      ...
      joystick3 = A0;//A5                                                          // joystick left / right
...
void loop() {                                                                   // executes forever until reset button is pressed or power is interrupted
  ...
  */
  ...
  oled.clear();  
  oled.setCursor(0,0);//pos row
  oled.println("Hello Loop!");
  oled.setCursor(0,2);//pos row
  counterloop++;
  oled.println(counterloop);
...
void pollJoystick() {
  ...
  uint8_t joyButtonState1 = digitalRead(joystick1);
  uint8_t joyButtonState2 = digitalRead(joystick2); 
  uint8_t joyButtonState3 = digitalRead(joystick3);
  uint8_t joyButtonState4 = digitalRead(joystick4);

  if (joyButtonState1 != joyOldButtonState1)
  {
    joyStickSpeed = JOYSTICK_SPEED;                                             // set the joystick speed back to normal
    if (millis () - joyButtonTime1 >= joyDebounceTime)
    {
      joyButtonTime1 = millis();                                                // when we closed the switch
      joyOldButtonState1 = joyButtonState1;                                     // remember for next time
      if (joyButtonState1 == LOW) {
        event = EVENT_ROTATE_CW;
      }
    }
  } else {
    if (joyButtonState1 == LOW) {                                               // joystick is being held in one position
      if (millis() - joyButtonTime1 >= joyStickSpeed) {
        joyButtonTime1 = millis();
        if (joyStickSpeed > 100) joyStickSpeed -= 20;
        event = EVENT_ROTATE_CW;
      }
    }
  }

  if (joyButtonState2 != joyOldButtonState2)
  {
    joyStickSpeed = JOYSTICK_SPEED;                                             // set the joystick speed back to normal
    if (millis () - joyButtonTime2 >= joyDebounceTime)
    {
      joyButtonTime2 = millis();                                                // when we closed the switch
      joyOldButtonState2 = joyButtonState2;                                     // remember for next time
      if (joyButtonState2 == LOW) {
        event = EVENT_ROTATE_CW;
      }
    }
  } else {
    if (joyButtonState2 == LOW) {                                               // joystick is being held in one position
      if (millis() - joyButtonTime2 >= joyStickSpeed) {
        joyButtonTime2 = millis();
        if (joyStickSpeed > 100) joyStickSpeed -= 20;
        event = EVENT_ROTATE_CW;
      }
    }
  ...
  if (joyButtonState3 != joyOldButtonState3)
  {
    joyStickSpeed = JOYSTICK_SPEED;                                             // set the joystick speed back to normal
    if (millis () - joyButtonTime3 >= joyDebounceTime)
    {
      joyButtonTime3 = millis();                                                // when we closed the switch
      joyOldButtonState3 = joyButtonState3;                                     // remember for next time
      if (joyButtonState3 == LOW) {
        event = EVENT_ROTATE_CC;
      }
    }
  } else {
    if (joyButtonState3 == LOW) {
      if (millis() - joyButtonTime3 >= joyStickSpeed) {
        joyButtonTime3 = millis();
        if (joyStickSpeed > 100) joyStickSpeed -= 20;
        event = EVENT_ROTATE_CC;
      }
    }
  ...
  if (joyButtonState4 != joyOldButtonState4)
  {
    joyStickSpeed = JOYSTICK_SPEED;                                             // set the joystick speed back to normal
    if (millis () - joyButtonTime4 >= joyDebounceTime)
    {
      joyButtonTime4 = millis();                                                // when we closed the switch
      joyOldButtonState4 = joyButtonState4;                                     // remember for next time
      if (joyButtonState4 == LOW) {
        event = EVENT_ROTATE_CC;
      }
    }
  } else {
    if (joyButtonState4 == LOW) {
      if (millis() - joyButtonTime4 >= joyStickSpeed) {
        joyButtonTime4 = millis();
        if (joyStickSpeed > 100) joyStickSpeed -= 20;
        event = EVENT_ROTATE_CC;
      }
    }
...
void switchToFindByGroup() {
  ...
  elements = GROUP_MENU_ELEMENTS;
  machineState = STATE_MENU_FIND_BY_GROUP;         
  DisplayToItem(groupMenu[position]);
...
void initSPI(void) {
  ...
  /* 
  #define SPI_SS_PRIMARY            22//11                                            // chip select primary (copy source)  (SPI)
  #define SPI_SS_SECONDARY          20//10                                            // chip select secondary (copy target)  (SPI)
  #define SPI_MOSI                  MOSI                                          // mosi (SPI)
  #define SPI_MISO                  MISO                                          // miso (SPI)
  #define SPI_SCK                   SCK                                           // clock (SPI)
  #define SPI_SPEED                 5000000UL                                     // 5MHz (max is 5MHz@3.3v)
...
void attachCommandCallbacks()                                                   // We must define a callback function in our Arduino program for each entry in the list below.
{
  ...
  cmdMessenger.attach(pyGetFirmwareVersion    , OnGetFirmwareVersion);
...
void OnGetFirmwareVersion() {
  setGreen();
  cmdMessenger.sendCmd(kStrAcknowledge, FIRMWARE_VERSION);                      // sending back FIRMWARE_VERSION 
  setPurple();  
}
